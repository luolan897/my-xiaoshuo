...
async function renderBookAiSettings() {
  ...
  $("#save-agent-tool-call-limit").addEventListener("click", async () => {
    ...
    button.disabled = true;
    try {
      await api(`/api/works/${state.work.id}/ai-settings`, { method: "PATCH", body: { agentToolCallLimit: Number($("#agent-tool-call-limit").value) } });
