...
当用户说“执行完整验收流程”或“执行完整验收合并流程”，或明确要求完成浏览器 E2E 验收、subagent 复审、修复 P1 以上问题和最终验收时，视为用户授权并要求执行以下流程。合并 PR 是条件步骤，不是启动该工作流的前置要求：

1. 在当前功能分支完成测试并提交改动。涉及 UI 时，必须启动使用隔离测试数据库的开发服务，通过内置浏览器自动化执行真实 E2E；覆盖本次功能的主要用户路径、桌面与窄屏关键视口、交互状态、键盘与焦点、控制台错误、失败网络请求、横向溢出、遮挡、重叠、截断及明显错位。截图只能作为辅助证据，必须同时验证实际交互和可见结果。
2. 若本次任务存在由当前 Agent 创建、目标为 `develop` 的待合并 PR，则直接合入 `develop`，无需再次询问；若不存在，则跳过该步骤。合并前必须确认 PR 范围、目标分支和必需检查状态；不得借此合并其他作者、其他任务、范围不明、存在冲突或检查失败的 PR，不得合入 `main`。无法安全合并时应报告具体阻塞，不得绕过保护规则。
3. 更新到远端最新 `develop`，并派发一个独立 subagent 对本次待验收范围进行 code review；存在已合并 PR 时，应以实际合并后的差异为准。复审必须关注正确性、回归、安全、权限与数据边界、事务和迁移、前端交互与可访问性、测试缺口，并按 P0、P1、P2、P3 标注严重级别，提供文件位置、复现条件和理由。
4. 必须解决复审提出的全部 P0 和 P1 问题；P2、P3 记录在最终结果中，由用户决定是否继续处理。每个可独立验证的问题按本文件要求单独修改、测试并提交。若没有 P0 或 P1，直接进入最终验收。
5. 若复审产生修复改动，则从最新 `develop` 派生修复分支，提交新的目标为 `develop` 的 PR；确认范围和必需检查通过后直接合入，无需再次询问。没有修复改动时跳过创建和合并 PR。不得向 `main` 发起修复 PR。
6. 在包含全部合并结果的最新 `develop` 上执行最终验收：运行与改动相关的测试、`npm test`、`npm run build`、健康检查和 SQLite 只读完整性检查；涉及 UI 时再次通过内置浏览器执行真实 E2E 和视觉布局检查，并确认控制台无新增错误、关键请求无异常。
7. 最终报告必须列出已合并 PR（如有）、subagent 复审结论及已修复的 P0/P1、测试与浏览器验收结果、数据库检查结果、仍存在的 P2/P3 或外部阻塞。只有完成上述适用步骤后，才能声明完整验收流程结束。

该快捷工作流只授予完成上述范围内测试、提交、创建 PR、合入 `develop` 和派发复审 subagent 的权限；不授予发布版本、创建 tag、创建 GitHub Release、合入 `main`、修改分支保护或忽略失败检查的权限。
