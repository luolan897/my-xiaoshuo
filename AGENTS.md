...
当用户说“执行完整验收合并流程”，或明确要求依次完成“浏览器 E2E 验收、合并本次 PR、合并后 subagent 复审、修复 P1 以上问题、再次合并并最终验收”时，视为用户授权并要求执行以下完整流程：
...
2. 将本次任务中由当前 Agent 创建、目标为 `develop` 的 PR 直接合入 `develop`，无需再次询问。合并前必须确认 PR 范围、目标分支和必需检查状态；不得借此合并其他作者、其他任务、范围不明、存在冲突或检查失败的 PR，不得合入 `main`。无法安全合并时应报告具体阻塞，不得绕过保护规则。
3. 所有本次 PR 合入后，更新到远端最新 `develop`，并派发一个独立 subagent 对实际合并后的差异进行 code review。复审必须关注正确性、回归、安全、权限与数据边界、事务和迁移、前端交互与可访问性、测试缺口，并按 P0、P1、P2、P3 标注严重级别，提供文件位置、复现条件和理由。
4. 必须解决复审提出的全部 P0 和 P1 问题；P2、P3 记录在最终结果中，由用户决定是否继续处理。每个可独立验证的问题按本文件要求单独修改、测试并提交。若没有 P0 或 P1，直接进入最终验收。
5. 修复从最新 `develop` 派生，提交新的目标为 `develop` 的 PR；确认范围和必需检查通过后直接合入，无需再次询问。不得向 `main` 发起修复 PR。
...
7. 最终报告必须列出已合并 PR、subagent 复审结论及已修复的 P0/P1、测试与浏览器验收结果、数据库检查结果、仍存在的 P2/P3 或外部阻塞。只有完成上述步骤后，才能声明“完整验收合并流程”结束。
