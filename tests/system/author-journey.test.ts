...
describe("作者完整创作流程", () => {
  ...
  it("首屏书架、大纲伏笔、续写守卫和关系银河图资源完整可达", async () => {
    ...
    expect(page.text).toContain('/app.js?v=20260728-balanced-node-colors-v1');
    expect(page.text).toContain('/styles.css?v=20260728-chapter-batch-controls-v1');
    ...
    expect(application.text).toContain('/relationship-graph.js?v=20260728-balanced-node-colors-v1');
    ...
    expect(application.text).toContain('class="task-auto-run-progress-ring ${runningCount ? "is-running" : "is-waiting"}" role="progressbar"');
