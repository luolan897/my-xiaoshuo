...
describe("作者完整创作流程", () => {
  ...
  it("首屏书架、大纲伏笔、续写守卫和关系银河图资源完整可达", async () => {
    ...
    expect(page.text).toContain('/styles.css?v=20260801-docx-export-v1');
    ...
    expect(page.text).toContain('<span class="settings-card-mark">DOC</span><strong>导出 DOCX</strong><small>导出 Word 正文；若已设置封面则嵌入为首页</small>');
    expect(application.text).toContain('id="work-export-button" class="ghost-button" type="button">下载 ZIP</button>');
    expect(application.text).toContain('id="work-export-docx-button" class="ghost-button" type="button">下载 DOCX</button>');
    expect(application.text).toContain("可导出 Markdown ZIP 或 DOCX 正文");
