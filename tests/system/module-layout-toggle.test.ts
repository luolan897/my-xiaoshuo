...
describe("知识模块布局切换", () => {
  ...
  it("设定、角色、种族、组织、伏笔与审核保留卡片并支持列表切换", async () => {
    ...
    expect(page.text).toContain('/styles.css?v=20260726-ai-history-icon');
    expect(page.text).toContain('/app.js?v=20260726-cache-hit-percent');
    expect(page.text).toContain('<script type="module" src="/app.js?v=20260726-cache-hit-percent"></script>');
