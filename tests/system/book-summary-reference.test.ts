...
describe("全书概要上下文引用", () => {
  ...
  it("默认不引用上下文并保留显式的章节与全书范围", async () => {
    ...
    expect(styles.text).toContain(".book-summary-context-percent-field input, .context-compact-threshold-field input, .agent-tool-call-limit-field input, .agent-tool-call-global-multiplier-field input { width: 64px; min-height: 32px; padding: 5px 8px; font-size: 13px;");
    expect(styles.text).toContain(".config-inline-save { display: flex; align-items: flex-end; gap: 10px;");
    expect(styles.text).toContain(".config-inline-save .agent-tool-call-limit-field,\n.config-inline-save .agent-tool-call-global-multiplier-field { display: grid; gap: 6px; width: 64px;");
